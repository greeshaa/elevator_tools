...
before_filter :signed_in_user
  ...
  def main
  	...
  	@infomessages 			 = []
    inspection_overdue   = Inspection.where(created_at: (Date.today - Time.now.to_a[7])..Date.today).where(next_inspection_at: (Date.today - Time.now.to_a[7])..(Date.today - 1.day))
